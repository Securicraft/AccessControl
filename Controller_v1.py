...
class CreateData:
    ...
    def CheckStateData(self):
        ...
        try:
            ...
            try:
                import socket;
                s = socket.create_connection((SQLHOST, SQLPORT), DOWNLOADTIME);
                s.close();
                del socket;
                self.cnx  = DB.connect( user     = LOCALUSER,
                                        password= LOCALPASS,
...
#Create variable for save privious RFID and present RFID
devices = [evdev.InputDevice(path) for path in evdev.list_devices()];
for dev in devices:
    if "RFID" in dev.name:
        print("Now we connected to RFID Reader: ", dev.name);
    else:
        print("Now we connected to Barcode Reader: ", dev.name);
